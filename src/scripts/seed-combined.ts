...
async function seed() {
  console.log("Seeding FlexTV dramas...");
  let count = 0;
  for (const drama of flexTVDramas) {
  ...
  console.log(`Seeded ${count} FlexTV dramas.`);

  console.log("Fetching FlickReels dramas...");
  const flickReelsDramas = await fetchFlickReelsHomepage();
  console.log(`Fetched ${flickReelsDramas.length} FlickReels dramas.`);

  let flickCount = 0;
  for (const d of flickReelsDramas.slice(0, 20)) {
    const url = `https://www.flickreels.net/`;
    await prisma.drama.upsert({
      where: { url: `${url}?playlet=${d.playlet_id}` },
      update: {
        title: d.title,
        platform: "FlickReels",
        tags: d.tags,
        chartType: flickCount % 2 === 0 ? "hot" : "rising",
  ...
  console.log(`Seeded ${flickCount} FlickReels dramas.`);

  console.log("Fetching NetShort dramas...");
  const netShortDramas = await fetchNetShortDramas();
  console.log(`Fetched ${netShortDramas.length} NetShort dramas.`);

  let netCount = 0;
  for (const d of netShortDramas.slice(0, 20)) {
    await prisma.drama.upsert({
      where: { url: d.url },
      update: {
        title: d.title,
        platform: "NetShort",
        tags: "Short Drama",
        chartType: netCount % 2 === 0 ? "hot" : "rising",
        score: Math.round((4.0 + Math.random() * 1.0) * 10) / 10,
        cover: d.cover,
  ...
  console.log(`Seeded ${netCount} NetShort dramas.`);

  console.log("Fetching MobiReels dramas...");
  const mobiReelsDramas = await fetchMobiReelsDramas();
  console.log(`Fetched ${mobiReelsDramas.length} MobiReels dramas.`);

  let mobiCount = 0;
  for (const d of mobiReelsDramas.slice(0, 20)) {
    await prisma.drama.upsert({
      where: { url: d.url },
      update: {
        title: d.title,
        platform: "MobiReels",
        tags: "Short Drama",
        chartType: mobiCount % 2 === 0 ? "hot" : "rising",
        score: Math.round((4.0 + Math.random() * 1.0) * 10) / 10,
        cover: d.cover,
        description: "",
      },
  ...
  console.log(`Seeded ${mobiCount} MobiReels dramas.`);

  console.log(`Total seeded: ${count + flickCount + netCount + mobiCount}`);
}
